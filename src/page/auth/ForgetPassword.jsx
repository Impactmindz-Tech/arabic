...
const ForgetPassword = () => {
    ...
    const { register, handleSubmit, formState: { errors } } = useForm({ resolver: yupResolver(verifyOtp) });
    const onSubmit = (data) => {
        navigate("/verify_otp")
        // try {
    ...
    return (
        <div className='2xl:max-w-[335px] w-full max-w-[270px] mx-auto h-full flex flex-col gap-2'>
            ...
            <form className="flex flex-col mt-10" onSubmit={handleSubmit(onSubmit)} noValidate>
                <label className="2xl:text-sm text-right  bg-white text-input-label -top-2 left-5 text-[11px] px-1" htmlFor="email">رقم الجوال</label>
                ...
                <p className="text-[11px] my-2 mr-2 text-input-placeholder w-max ml-auto" to='/auth/forgot-password'>نسيت كلمة المرور</p>
